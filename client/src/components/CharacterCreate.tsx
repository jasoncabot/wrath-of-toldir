...
import { imageForEntity, playableEntities } from '../assets/spritesheets/Sprites';
...
export const CharacterCreate = (props: { onNewCharacter: (name: string, texture: EntityTexture) => void }) => {
    ...
    const onCreateSelected = () => {
        ...
        props.onNewCharacter(name, playableEntities[textureIndex]);
    }
                ...
                <div className="w-64 h-64 max-w-xs overflow-hidden rounded-lg shadow-md bg-white hover:shadow-xl transition-shadow duration-300 ease-in-out">
                    <form className="flex flex-col w-full h-full max-w-xs p-4" autoComplete='off' onSubmit={(e: any) => {
                        ...
                        <div className='flex flex-1 items-center'>
                            ...
                            <img className='object-none object-left-top h-12' src={imageForEntity(playableEntities[textureIndex])} alt={`Hero ${textureIndex}`} width={48} height={48} />
                            <a className='flex flex-1 items-center cursor-pointer h-full justify-end select-none' onClick={() => setTextureIndex(Math.min(playableEntities.length - 1, textureIndex + 1))}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" viewBox="0 0 16 16">
